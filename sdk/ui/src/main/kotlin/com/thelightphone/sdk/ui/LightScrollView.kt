...
private const val SCROLLBAR_SHOW_DEBOUNCE_MS = 200L
...
fun LightScrollView(
    ...
    // Debounced: right after this screen enters, layout can briefly settle
    // into a smaller-than-final size (e.g. mid entrance-transition), which
    // makes short content appear to overflow for a frame or two before
    // resolving. Waiting a beat before showing the bar avoids a flash for
    // content that never actually needed to scroll; hiding stays instant.
    var showScrollBar by remember { mutableStateOf(false) }
    LaunchedEffect(rawShowScrollBar) {
        if (rawShowScrollBar) {
            delay(SCROLLBAR_SHOW_DEBOUNCE_MS)
            showScrollBar = true
        } else {
            showScrollBar = false
        }
